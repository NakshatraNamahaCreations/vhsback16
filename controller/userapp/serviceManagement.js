...
class serviceManagement {
  ...
  async updateServices(req, res) {
    try {
      ...
      if (file1) {
        findService.Desimg = file1;
      }

      if (file2) {
        findService.Inimg = file2;
      }
  ...
  async getServiceByCategory(req, res) {
    try {
      ...
      let data = await serviceManagementModel
        .find({ category }, "serviceName Subcategory")
        .sort({ _id: -1 });
  ...
  async getServicedetailswithlazyloading(req, res) {
    try {
      ...
      const servicedetails = await serviceManagementModel
        .find({})
        .sort({ _id: -1 })
        .skip(skip)
        .limit(limit);
      ...
      if (servicedetails.length > 0) {
        return res.json({ servicedetails: servicedetails, currentPage: page });
      } else {
